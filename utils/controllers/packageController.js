...
const getPackages = catchAsync(async (req, res, next) => {
  ...
  const packages = await PackageModel.find(query).select(unselectedLang);
...
const getPackageById = catchAsync(async (req, res, next) => {
  ...
  if (lang === 'both') {
    ...
    packageData = await PackageModel.findById(packageId).select(`-${unselectedLang}`);
  }
...
const updatePackage = catchAsync(async (req, res, next) => {
  ...
  const packageData = await PackageModel.findByIdAndUpdate(packageId, req.body, { new: true, runValidators: true });
  if (!packageData) return next(new AppError('Not Found', 404));
