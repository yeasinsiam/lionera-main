...
const getSlideshows = catchAsync(async (req, res, next) => {
  ...
  const slideshows = await SlideshowModel.find(query).select(unselectedLang).populate('package', unselectedLang);
...
const getSlideshowById = catchAsync(async (req, res, next) => {
  ...
  if (lang === 'both') {
    ...
    slideshow = await SlideshowModel.findById(slideshowId).select(`-${unselectedLang}`);
  }
...
const updateSlideshow = catchAsync(async (req, res, next) => {
  ...
  const slideshow = await SlideshowModel.findByIdAndUpdate(slideshowId, req.body, { new: true, runValidators: true });
  if (!slideshow) return next(new AppError('Not Found', 404));
