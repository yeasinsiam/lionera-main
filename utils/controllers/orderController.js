...
const getOrders = catchAsync(async (req, res, next) => {
  ...
  const orders = await OrderModel.find().select(`${unselectedLang}`)
...
const getOrderAddonAndForms = catchAsync(async (req, res, next) => {
  ...
  if (moment().diff(order.createdAt, 'h') > 24) return next(new AppError('Time Expired', 400))
...
const insertOrUpdateOrderFormData = catchAsync(async (req, res, next) => {
  ...
  const formData = await FormDataModel.findOneAndUpdate({ order: orderId }, req.body, { new: true, runValidators: true, upsert: true })
...
const getOrderById = catchAsync(async (req, res, next) => {
  ...
  const order = await OrderModel.findById(req.params.orderId).select(`${unselectedLang}`)
    .populate('occasion', `${unselectedLang}`)
    .populate('design', `${unselectedLang}`)
    .populate('slideshow', `${unselectedLang}`)
    .populate('package', `${unselectedLang}`)
