...
const getOccasionsById = catchAsync(async (req, res, next) => {
  ...
  const occasion = await OccasionModel.findById(req.params.occasionId).select(unselectedLang);
...
const updateOccasion = catchAsync(async (req, res, next) => {
  ...
  const occasion = await OccasionModel.findByIdAndUpdate(occasionId, req.body, { new: true, runValidators: true });
